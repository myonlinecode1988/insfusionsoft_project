...
topic. The message handle (which acts like a subscriber) listens from the topic,transforms data (adds NULL where necessary) and inserts the data as a row in the chosen table. 
...
```
We have to run the script each time we want to generate new data (see demo)
```
